...
Tasks to publish the monorepo to GitHub and deploy both backend (Go) and frontend (Vite/React) to Google Cloud Run.
...
- If using Cloud SQL or Memorystore, add a Serverless VPC connector and `--add-cloudsql-instances` or private IP; set `DATABASE_URL`/`REDIS_URL`.
